...
export const projects = mysqlTable(
  "projects",
  {
    ...
    budgetDevelopment: decimal("budget_development", { precision: 10, scale: 2 }).notNull().default("0"),
    budgetQa: decimal("budget_qa", { precision: 10, scale: 2 }).notNull().default("0"),
    budgetManagement: decimal("budget_management", { precision: 10, scale: 2 }).notNull().default("0"),
    createdAt: datetime("created_at").notNull().$defaultFn(() => new Date()),
  },
  (table) => [
...
export const timeEntries = mysqlTable(
  "time_entries",
  {
    ...
    workType: mysqlEnum("work_type", ["DEVELOPMENT", "QA", "MANAGEMENT"]).notNull().default("DEVELOPMENT"),
    description: text("description"),
    createdAt: datetime("created_at").notNull().$defaultFn(() => new Date()),
  },
  (table) => [
...
export const leaveRequests = mysqlTable(
  "leave_requests",
  {
    ...
    hours: decimal("hours", { precision: 5, scale: 2 }).notNull().default("8"),
    reason: text("reason"),
    status: mysqlEnum("status", ["PENDING", "PARTIALLY_APPROVED", "APPROVED", "REJECTED"]).notNull().default("PENDING"),
    reviewedBy: varchar("reviewed_by", { length: 36 }),
    reviewedAt: datetime("reviewed_at"),
    reviewedBy2: varchar("reviewed_by_2", { length: 36 }),
    reviewedAt2: datetime("reviewed_at_2"),
